...
get() {
   ...
   local option_value_global="$(tmux show-option -gqv "$option")"

   if [ "$option_value" ]; then
      echo "$option_value"
   elif [ "$option_value_global" ]; then
      echo "$option_value_global"
   else
      echo "$default_value"
   fi
}
...
onedark_accient_color=$(get "@onedark_accient_color" "$onedark_green")

set "pane-border-fg" "$onedark_white"
set "pane-border-bg" "$onedark_black"
set "pane-active-border-fg" "$onedark_accient_color"
...
set "@prefix_highlight_bg" "$onedark_accient_color"
set "@prefix_highlight_copy_mode_attr" "fg=$onedark_black,bg=$onedark_accient_color"
...
set "status-right" "#[fg=$onedark_white,bg=$onedark_black,nounderscore,noitalics]${time_format}  ${date_format} #[fg=$onedark_visual_grey,bg=$onedark_black]#[fg=$onedark_visual_grey,bg=$onedark_visual_grey]#[fg=$onedark_white, bg=$onedark_visual_grey]${status_widgets} #[fg=$onedark_accient_color,bg=$onedark_visual_grey,nobold,nounderscore,noitalics]#[fg=$onedark_black,bg=$onedark_accient_color,bold] #h #[fg=$onedark_yellow, bg=$onedark_accient_color]#[fg=$onedark_red,bg=$onedark_yellow]"
set "status-left" "#[fg=$onedark_black,bg=$onedark_accient_color,bold] #S #{prefix_highlight}#[fg=$onedark_accient_color,bg=$onedark_black,nobold,nounderscore,noitalics]"
